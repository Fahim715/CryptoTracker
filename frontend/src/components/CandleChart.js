...
const CustomTooltip = ({ active, payload }) => {
  ...
  return (
    <div style={{ background: '#0d1220', border: '1px solid #1a2236',
      ...
      <div style={{ color: '#546e8a', marginBottom: 4 }}>{fmt(new Date(d.openTime).getTime())}</div>
...
export default function CandleChart({ data, color = '#00e5ff' }) {
    ...
    <ResponsiveContainer width="100%" height={200}>
      <ComposedChart data={chartData} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
        ...
        <XAxis dataKey="openTime"
          tickFormatter={v => fmt(new Date(v).getTime())}
